...
void Change_address(int nos, uint8_t current_id, uint8_t new_id){ // nos: number of sensor -> not included
    uint8_t frame[8];
    frame[0] = current_id;
    frame[1] = 0x06; // write
    frame[2] = 0x07;
    frame[3] = 0xD0;
    frame[4] = 0x00;
    frame[5] = new_id;// new id

    uint16_t crc = CRC_Cal(frame,6);
    frame[6] = (uint8_t)(crc & 0xFF); // low byte
    frame[7] = (uint8_t)((crc >> 8) & 0xFF); // high byte

    uart_flush(UART_PORT);

    int ret = uart_write_bytes(UART_PORT,(const char*)frame,sizeof(frame));
    if(ret <= 0){
        printf("Chane ID failed \n");
    }
...
int Change_Baudrate(uint8_t slave_id, uint8_t baud_index) {
    uint8_t frame[8];
    // baud_index = 0x00: 2400, 0x01 = 4800, 0x02 = 9600
    frame[0] = slave_id;
    frame[1] = 0x06; // Write Single Register
    frame[2] = 0x07; // Register High
    frame[3] = 0xD1; // Register Low 
    frame[4] = 0x00; // Value High
    frame[5] = baud_index; // Value Low
...
void Function_write_coeff_calib(uint8_t slave_id, uint8_t coeffi_address, float value){  // Only for coefficient and calib address
    int16_t value_int = (int16_t)(value * 100.0f);
    uint8_t frame[8];//response[7]; // Just read 1 reg so 7 bytes response
    frame[0] = slave_id;
    frame[1] = 0x06; // write
    frame[2] = 0x00;
    frame[3] = coeffi_address;
    frame[4] = (uint8_t)((value_int >> 8) & 0xFF); // high byte
...
void Function_read_value(uint8_t slave_id, uint16_t reg) {
    ...
    frame[0] = slave_id;
    frame[1] = 0x03;
    frame[2] = (uint8_t)((reg >> 8) & 0xFF); // high
    frame[3] = (uint8_t)(reg & 0xFF);
    frame[4] = 0x00;
    frame[5] = 0x01; // Đọc 1 thanh ghi
...
void Read_Sensor(uint8_t slave_id, uint8_t Things){ // Things is that u want to return
    uint8_t frame[8];
    uint8_t response[(Things + 1)*2 + 5];
    frame[0] = slave_id;
    frame[1] = 0x03; // read function
    frame[2] = 0x00; // starting address
    frame[3] = 0x00; // starting address
    frame[4] = 0x00; // Number of register
    frame[5] = Things + 1;
