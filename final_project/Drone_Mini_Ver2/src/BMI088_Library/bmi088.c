...
int BMI088_Calib(void){
	...
	for(int i = 0; i < 550; i++){
		...
		if(i > 49){
			gx = (int16_t)(((uint16_t)gyro[1] << 8) | gyro[0]);
            gy = (int16_t)(((uint16_t)gyro[3] << 8) | gyro[2]);
            gz = (int16_t)(((uint16_t)gyro[5] << 8) | gyro[4]);

			sum_gx += gx;
			sum_gy += gy;
			sum_gz += gz;

			ax = (int16_t)(((uint16_t)accel_data[1] << 8) | accel_data[0]);
            ay = (int16_t)(((uint16_t)accel_data[3] << 8) | accel_data[2]);
            az = (int16_t)(((uint16_t)accel_data[5] << 8) | accel_data[4]);
	...
	bmi088_offset.acc_offset_z = (sum_az / 500.0f) - ACC_LSB_6G; // nằm ngửa -> đúng || xem khi debug thi +
...
void BMI088_Data(uint8_t *acc_data, uint8_t *gyro_data){
    ...
    ACC_X = (int16_t)(((uint16_t)acc_data[1] << 8) | acc_data[0]);
    ACC_Y = (int16_t)(((uint16_t)acc_data[3] << 8) | acc_data[2]);
    ACC_Z = (int16_t)(((uint16_t)acc_data[5] << 8) | acc_data[4]);

    GYRO_X = (int16_t)(((uint16_t)gyro_data[1] << 8) | gyro_data[0]);
    GYRO_Y = (int16_t)(((uint16_t)gyro_data[3] << 8) | gyro_data[2]);
    GYRO_Z = (int16_t)(((uint16_t)gyro_data[5] << 8) | gyro_data[4]);
...
void Calculate_And_Filter_Angle(uint8_t *acc_data,uint8_t *gyro_data,float dt){
	...
	float acc_pitch = atan2f(-final.ax,sqrtf(final.ay*final.ay + final.az*final.az))*(180/M_PI);
