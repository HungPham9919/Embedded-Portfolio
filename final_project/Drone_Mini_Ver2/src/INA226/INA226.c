...
int INA226_Initialized(void){
    ...
    if(i2c_write_data(dev_i2c1,INA226_ADDR, INA226_Config_Reg, 0x4127, 2,&dma1_stream6_signal) != 0) goto ERR;
    // first state 3.9V/0.00125 = 3120 || 3.6 2880 || 3.4 2720
    Danger_Voltage = 0x0C30; // 3120 DEC
    if(i2c_write_data(dev_i2c1,INA226_ADDR, INA226_Alert_limit, Danger_Voltage, 2,&dma1_stream6_signal) != 0) goto ERR;;
    // Mask enable BUL bit 12
    if(i2c_write_data(dev_i2c1,INA226_ADDR, INA226_Mask_Enable, 0x1001, 2,&dma1_stream6_signal) != 0) goto ERR; // Alert on
